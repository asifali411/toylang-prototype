use crate::{errors::{interpreter_error::InterpreterError, lang_error::IResult}, interpreter::{Interpreter, value::Value}, lexer::token::TokenKind, parser::expression::Expr};

impl Interpreter {
  pub(crate) fn eval_assign(&mut self, expr: &Expr, name: &String, value: &Box<Expr>, line: &usize, col: &usize) -> IResult<Value> {
  ...
  pub(crate) fn lookup_var(&self, name: &str, expr: &Expr, line: usize, col: usize) -> IResult<Value> {
    if let Some(depth) = self.locals.get(&(expr as *const Expr)).copied() {
