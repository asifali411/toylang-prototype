...
use crate::{errors::{interpreter_error::InterpreterError, lang_error::IResult}, interpreter::{Interpreter, class::class::Class, function::Function, value::Value}, lexer::token::{Token, TokenKind}, parser::{expression::Expr, statement::Stmt}};

impl Interpreter {
  pub(crate) fn eval_class_statement(&mut self, class_name: &str, methods: &Vec<Stmt>, superclass: &Option<Expr>) -> IResult<Value> {
  ...
  pub(crate) fn eval_get(&mut self, object: &Expr, name: &String, line: &usize, col: &usize) -> IResult<Value> {
  ...
  pub(crate) fn eval_set(&mut self, object: &Expr, name: &String, value: &Expr) -> IResult<Value> {
    match self.eval_expression(object)? {
        Value::OBJECT(obj) => {
